 ...
 *   clang -std=c11 -Wall -Wextra -Wpedantic -I../../../include -c dynamic_kv_length_fixed.c
 *
 * License note:
...
SPS_ENTRY("dynamic_kv_length_fixed")
SPS_RETURN_OUTPUT("return")
uint32_t dynamic_kv_length_fixed(
    uint32_t secret_length SPS_COMPONENT("secret-length"),
    uint32_t private_result SPS_COMPONENT("private-result"),
    uint32_t *public_allocation_count SPS_ROOT("public-allocation-count"),
    uint32_t *public_iteration_count SPS_ROOT("public-iteration-count")) {
  (void)secret_length;

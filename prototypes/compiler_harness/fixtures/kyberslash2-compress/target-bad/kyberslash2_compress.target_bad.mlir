// RUN: %checkpoint-runner run --snapshot fixtures/kyberslash2-compress/target-bad/snapshot.yaml --pipeline modeled-shape --endpoint %t.modeled.mlir --records %t.checkpoints -- %mlir-opt %s -o %t.modeled.mlir
// RUN: %checkpoint-runner finalize --test fixtures/kyberslash2-compress/target-bad/kyberslash2_compress.target_bad.mlir --records %t.checkpoints
...
module {
  llvm.func @kyberslash2_compress_target_bad(
      %coefficient: i16 {
  ...
  ^nonzero:
    ...
    %shifted = llvm.shl %coefficient32, %four : i32
    %numerator = llvm.add %shifted, %round : i32
    %quotient = llvm.udiv %numerator, %q : i32
    %compressed32 = llvm.and %quotient, %fifteen : i32
    %compressed8 = llvm.trunc %compressed32 : i32 to i8
    llvm.return %compressed8 : i8
  }
}

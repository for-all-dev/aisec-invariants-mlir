 ...
 *   Written for this harness. No upstream body is copied. This reduces no single
 *   incident; the mechanism is a default-enabled backend pass.
 *
 * Secret inputs:
 *   secret
 *
 ...
 *   Every other bad fixture here describes a leak a PROGRAMMER wrote, present in
 *   the source and findable by review. This leak is absent from the source and
 ...
 *   clang -O2 --target=x86_64-unknown-linux-gnu -S launder_scan_bad.c
...
uint64_t launder_scan_bad(int secret, uint64_t x, const uint64_t *p)
{
  ...
  return secret ? v : x;  /* select with a memory operand -> forcibly branched on x86 */
}

 ...
 *     LLVM IR       : 0 select, 0 br i1        -- the mask survives as arithmetic
 *     x86-64 asm    : 0 conditional jumps      -- negl/sbbq/andq, unconditional load
 *     aarch64 asm   : 0 conditional branches
 *
 ...
 *   backend then converts to a branch. The barrier makes the mask opaque, so the
 *   pattern match fails and the arithmetic is emitted as written.
 *
 ...
 *
 * Honest limitation:
 *   Inline-asm opacity is a convention, not a specification. Nothing in the C or
 *   LLVM standards requires an empty asm block to survive; it survives because
 ...
 *   clang -O2 --target=x86_64-unknown-linux-gnu -S launder_scan_fixed.c
 *
 * License note:

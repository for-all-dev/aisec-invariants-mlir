 ...
 * corpus stays linkable: c/Makefile compiles and links the discovered fixture
 * sources into the equivalence driver, so an undefined symbol breaks
 * check-equivalence.
 *
 ...
 */
unsigned sps_release_policy_h_candidate(unsigned raw)
{
  return raw;
}

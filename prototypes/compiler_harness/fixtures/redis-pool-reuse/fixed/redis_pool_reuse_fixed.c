 ...
 *   clang -std=c11 -Wall -Wextra -Werror -I../../../include -c redis_pool_reuse_fixed.c
 *
 * License note:
...
SPS_ENTRY("redis_pool_reuse_fixed")
SPS_RETURN_OUTPUT("return")
uint32_t redis_pool_reuse_fixed(
    uint32_t response_owned_by_a SPS_COMPONENT("response-owned-by-a"),
    uint32_t response_owned_by_b SPS_COMPONENT("response-owned-by-b"),
    uint32_t request_a_was_cancelled SPS_COMPONENT("request-a-was-cancelled")) {

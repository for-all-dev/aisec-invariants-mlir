...
//
// RETIRED REV4.0/V2 EXPERIMENT. Every outlined-wrapper rule discussed below
// describes the legacy carrier experiment only. Under SPS-LLVM-NF-v2 the sole
// carrier is llvm.sps.release, so this module is an NFv2-negative
// ReleaseCarrierMismatch before its legacy wrapper-survival question arises.
//
...
//
// WHAT HAPPENED. The policy declared one release `p_invalid_callable` implemented by an
...
//
//   mlir-opt fixtures/release-carrier/pinned-control/release_carrier_pinned.control.mlir --inline --cse
//
// on a variant whose wrapper lacks the pinning attributes.
//
// WHY THIS IS A CARRIER DEFECT AND NOT A LEAK. The invalid callable design
// section 4.4 required a
...
//    sps.release_policy = "p_invalid_callable". REL-01 forbids "a unit attribute whose
...
module {
  llvm.func @release_carrier_lost_bad(
    ...
    // reason: NFv2 has no llvm.sps.release; the callable scaffold also lost its outlined wrapper

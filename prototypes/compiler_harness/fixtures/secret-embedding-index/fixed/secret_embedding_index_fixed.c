 ...
 *   clang -std=c11 -Wall -Wextra -Wpedantic -I../../../include -c secret_embedding_index_fixed.c
 *
 * License note:
...
SPS_ENTRY("secret_embedding_index_fixed")
SPS_RETURN_OUTPUT("return")
uint32_t secret_embedding_index_fixed(
    const uint32_t table[16] SPS_ROOT("table"),
    uint32_t secret_index SPS_COMPONENT("secret-index")) {
  uint32_t selected = 0;
  secret_index &= 15u;

 ...
 *   clang -std=c11 -Wall -Wextra -Werror -I../../../include -c secret_logging_checkpoint_fixed.c
 *
 * License note:
...
SPS_ENTRY("secret_logging_checkpoint_fixed")
void secret_logging_checkpoint_fixed(
    uint32_t service_account_token SPS_COMPONENT("service-account-token"),
    uint32_t *private_state SPS_ROOT("private-state"),
    uint32_t *public_log SPS_ROOT("public-log"),
    uint32_t *public_checkpoint SPS_ROOT("public-checkpoint")) {

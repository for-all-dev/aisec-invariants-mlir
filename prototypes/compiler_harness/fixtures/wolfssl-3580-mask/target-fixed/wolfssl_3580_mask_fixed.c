 ...
 *   riscv32-gcc -I../../../include -O3 -march=rv32i -mabi=ilp32 -S wolfssl_3580_mask_fixed.c
 *
 * License note:
...
__attribute__((noinline)) static uint32_t ct_eq_mask(uint32_t a, uint32_t b) {
  uint32_t x = a ^ b;
  x |= 0u - x;
  return 0u - ((x >> 31) ^ 1u);
}

SPS_ENTRY("wolfssl_3580_select_fixed")
SPS_RETURN_OUTPUT("return")
__attribute__((noinline)) uint32_t
wolfssl_3580_mask_fixed(const uint32_t table[16] SPS_ROOT("table"),
                        uint32_t table_index SPS_COMPONENT("table-index")) {
  uint32_t result = 0;
